...
Dodecahedron::Bigint sum, multiplier;
std::string inpstring, numSegment;
std::string sandiAyam[21] = {"0", ".", "..", "...", "....", "-", ".-", "..-", "...-", "....-", "--", ".--", "..--", "...--", "....--", "---", ".---", "..---", "...---", "....---"};
std::vector<std::string> numVect;

int main(){
    sum = 0;
    numSegment = "";
    while(getline(std::cin, inpstring)){
        if(inpstring != ""){
            numSegment = numSegment+inpstring;
        }else{
            numVect.push_back(numSegment);
            numSegment = "";
        }
    }
    numVect.push_back(numSegment);
    std::reverse(numVect.begin(), numVect.end());
    for(int i = 0; i < numVect.size(); i++){
        for(int i2 = 0; i2 < 21; i2++){
            if(numVect[i] == sandiAyam[i2]){
                if(i == 2){
                    multiplier = 360;
                }else if(i < 2){
                    multiplier = Dodecahedron::pow(20, i);
                }else if(i > 2){
                    multiplier = static_cast<Dodecahedron::Bigint>(360)*Dodecahedron::pow(20, i-2);
                }
                sum += static_cast<Dodecahedron::Bigint>(i2)*multiplier;
                break;
            }
        }
    }
